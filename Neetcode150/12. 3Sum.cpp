...
public:
    ...
    vector<vector<int>> threeSum(vector<int> &nums)
    {
        ...
        for (int i = 0; i < n; i++)
        { // for more optimze we can check it for (n-2) becuase we just consider i (n1) to 3rd last element in array not more than that for (int i=0; i<n-2; i++)

            if (i > 0 && nums[i] == nums[i - 1])
            { // at 0 index there's nothing (i-1) so we'll check if n>0
                continue;
            }
